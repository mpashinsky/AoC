var pinput = `...................15....904...........850.................329...................13....................................871....816....697....
...
for(let y=0; y< inputArr.length; y++) {
    ...
    for(let x=0; x< inputArr[y].length; x++) {
        ...
        if(part === 1) {
            if (inputArr[y][x] !== '.' && !/^\d+$/.test(inputArr[y][x])) {
                starCoords.push({x, y})
            }
        }
        else {
...
var partCoords = starCoords.map(i=>getPartNumberCoords(i))

var partNumbers = []
if(part === 1) {

    partNumbers = partCoords.map(i=> i.map(getNumberByCoord))
    console.log(arrSum(partNumbers.map(i=>arrSum(i))))
}
else {
    partNumbers = partCoords.map(i=> i.map(getNumberByCoord)).filter(i=>i.length>1)
    console.log(arrSum(partNumbers.map(i=>i[0]*i[1])))
}

function getNumberByCoord(coord) {

    return Number(inputArr[coord.y].slice(coord.x).match(/^\d+/)[0])
}


function getPartNumberCoords(coord) {

    var numCoords = new Map()
    for(let y=coord.y-1; y<= coord.y+1; y++) {

        if(inputArr[y]===undefined) continue
        for(let x=coord.x-1; x<= coord.x+1; x++) {

            if(inputArr[y][x] === undefined) continue
            if(/^\d+$/.test(inputArr[y][x])) {
                let numStartCoords = findNumCoords(x, y)
                if(!numCoords.has(`${numStartCoords.x},${numStartCoords.y}`)) {
                    numCoords.set(`${numStartCoords.x},${numStartCoords.y}`, ({x:numStartCoords.x,y:numStartCoords.y}))
                }
            }
        }
    }
    return Array.from(numCoords.values())
}

function findNumCoords(x, y) {

    var index = x
    while(index>=0) {

        let symb = inputArr[y][index]
        if(/^\d+$/.test(symb)) {
            index--
        }
        else {
