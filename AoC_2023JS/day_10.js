var pinput = `J.7--J-.LL77FJ-F7FLJ-7.|FFF.F7FL-7F-7.L-FF77.-F-L7.F-L--7FL7..LF|J.7FF7.FLJ.-7-F7F-J77F77.|7.777FJ7FL7F-|.FFJ7|-|7FL-.LL7F77.|J7.|JJ-7-7|-|7
...
L7.FL-J-|-7-|L-JL7F7.LJFJF-7FJL--7||LJF---7F7||F7F7||L-7L7|L-JLJ||L-7L-7|F7F-J|||L7F-JF7LJL7LJLJLJL7FJL--J|F-7F7F7F---JLJL-J|FJ|F-J|F-J7.F|J
...
pinput.split('\n').map((i, index)=>i.indexOf('S') !== -1 ? console.log(i.indexOf('S')):0)
...
var inputArr = pinput.split('\n').map(i=>i.split('').map(i=>dict[i].split('\n').map(i=>i.trim())))
var cnt = 0

//var coords= {x:4,y:4}
var coords= {x:187,y:271}
var queue=  []

inputArr = inputArr.map(item => {

    var res = new Array(item[0].length)
    for(let i=0; i<item[0].length;i++) {

        res[i] = item.reduce((a,b) => a+b[i], '')
    }
    return res

})
var fieldMap = [].concat.apply([], inputArr).map(i=>i.split(''))
//fieldMap[4][4] = 2
console.log(fieldMap)

var visited = new Array(fieldMap.length).fill([]).map(i=>new Array(fieldMap[0].length).fill(0))
console.log(visited)
...
function bfs(graph, start) {
    ...
    while (queue.length) {
        ...
        if (!visited[vertex.y][vertex.x]) {
            visited[vertex.y][vertex.x] = 1;

            var nbh = getNeighbours(vertex)
                if(nbh.length === 0/* && queue.length === 0*/) {
                    return vertex.dist
                }
                nbh.forEach(item=>queue.push({x:item.x,y:item.y,dist:vertex.dist+1}))
        }
        //console.log(visited)
    }
...
function getNeighbours(coords) {
    ...
    if(fieldMap[coords.y][coords.x-1] === '1' && visited[coords.y][coords.x-1] === 0) {
        res.push({x:coords.x-1,y:coords.y})
    }
    if(fieldMap[coords.y][coords.x+1] === '1' && visited[coords.y][coords.x+1] === 0) {
        res.push({x:coords.x+1,y:coords.y})
    }
    if(fieldMap[coords.y-1][coords.x] === '1' && visited[coords.y-1][coords.x] === 0) {
        res.push({x:coords.x,y:coords.y-1})
    }
    if(fieldMap[coords.y+1][coords.x] === '1' && visited[coords.y+1][coords.x] === 0) {
        res.push({x:coords.x,y:coords.y+1})
    }
