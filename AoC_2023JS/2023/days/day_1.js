...
var wordNumbers = "one, two, three, four, five, six, seven, eight, nine".split(', ')

var isPart2 = false
//isPart2 = true
console.log(arrSum(pinput.split('\n').map(i=>findFirstNumber(i)+findLastNumber(i)).map(Number)))

function findFirstNumber(str) {

    for(let i=0; i< str.length; i++) {
        const tmp = str.slice(i)
        let number = isNumber(tmp);
        if(number !== -1)
...
function isNumber(str) {
    const firstCharAsNumber = Number(str[0]);

    if (!isNaN(firstCharAsNumber)) {

        return firstCharAsNumber;
    }

    const wordNumber = isWordNumber(str);

    return wordNumber.length > 0 ? wordNumber : -1;
}

function isWordNumber(str) {
    const foundIndex = wordNumbers.findIndex((word) => str.startsWith(word));
