var pinput = `Game 1: 10 green, 5 blue; 1 red, 9 green, 10 blue; 5 blue, 6 green, 2 red; 7 green, 9 blue, 1 red; 2 red, 10 blue, 10 green; 7 blue, 1 red
...
console.log(arrSum(pinput.split('\n').map((i, index) => parseGamePart1(i, index))))
console.log(arrSum(pinput.split('\n').map((i, index) => parseGamePart2(i, index))))

function parseGamePart1(str, index) {

    var maxRed = Math.max(...str.match(/\d+ red/g).map(getNum)) <= 12
    var maxGreen = Math.max(...str.match(/\d+ green/g).map(getNum)) <= 13
    var maxBlue = Math.max(...str.match(/\d+ blue/g).map(getNum)) <= 14

    return maxRed && maxGreen && maxBlue ? index + 1 : 0;
}

function parseGamePart2(str, index) {

    var maxRed = Math.max(...str.match(/\d+ red/g).map(getNum))
    var maxGreen = Math.max(...str.match(/\d+ green/g).map(getNum))
    var maxBlue = Math.max(...str.match(/\d+ blue/g).map(getNum))
